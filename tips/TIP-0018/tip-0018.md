...
author: Levente Pap (@lzpap) <levente.pap@iota.org>, Vlad Semenov <vlad.semenov@iota.org>
...
##### State Controllers Addresses Unlock Condition
...
machine that can only be carried out by one of the `State Controllers Addresses`, hence the distinct unlock condition type.

<details>
    <summary>State Controllers Addresses Unlock</summary>
    <blockquote>
        Defines the State Controllers Addresses that own this output, that is, one state controller can unlock it with the proper <i>Unlock</i> in a
...
<table>
    ...
    <tr>
        ...
        <td>uint8</td>
        <td>
            Set to <strong>value 4</strong> to denote an <i>State Controllers Addresses Unlock Condition</i>.
        </td>
    </tr>
    <tr>
    ...
    <details>
        ...
        <table>
            ...
            <tr>
                ...
                <td colspan="2">
                    <details>
                        <summary>State Controllers Addresses Unlock Condition</summary>
                        <table>
                            <tr>
                                <td><b>Name</b></td>
                                <td><b>Type</b></td>
...
- `Unlock Condition Type` of an <i>Unlock Condition</i> must define one of the following types:
  - <i>State Controllers Addresses Unlock Condition</i>
...
- `Address`es of <i>State Controllers Addresses Unlock Condition</i> and `Address` of
  <i>Governor Address Unlock Condition</i> must be different from the alias address derived from `Alias ID`.
- `Addresses Count` of <i>State Controllers Addresses Unlock Condition</i> must satisfy `0` < `Addresses Count` ≤ `Max State Controllers Count`.
- `Addresses` list of <i>State Controllers Addresses Unlock Condition</i> must contain `Addresses Count` entries.
- `Addresses` of <i>State Controllers Addresses Unlock Condition</i> must be pairwise unique and must be sorted in lexicographical order of their serialized form.
...
- State transition:
    ...
    - The unlock must correspond to one `Address` of <i>State Controllers Addresses Unlock Condition</i>.
...
- Governance transition:
    ...
    - Governance transition must only change the following fields:
       - `Address` of <i>State Controllers Addresses Unlock Condition</i>,
...
#### Unlocking Alias Output locked by a State Controller Address

Alias Output with a state transition can be unlocked by a State Controller.
Depending on State Controller Address Type the corresponding unlock block will be either Signature Unlock, or Alias
Unlock, or NFT Unlock. After the unlock block is verified the resulting UTXO address is then looked up in the list of
`State Controllers Addresses`. If the UTXO address is not found then the unlock block is invalid. The lookup operation
is reasonably efficient in a sorted list and takes `O(log n)` comparison steps or can be done in `O(1)` using hash
tables.

The resulting UTXO address for Signature Unlock is computed as BLAKE2b-256 hash of the Ed25519 `Public key`,
for Alias Unlock it is the address of an Alias Output referenced by `Alias Reference Unlock Index`,
for NFT Unlock it is the address of an NFT Output referenced by `NFT Reference Unlock Index`.
